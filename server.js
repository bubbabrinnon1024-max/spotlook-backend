...
app.get("/admin/timmy-dev-backdoor-9x2k", function(req, res) {
  res.send("<!DOCTYPE html><html><head><meta charset='UTF-8'/><meta name='viewport' content='width=device-width,initial-scale=1'/><title>SpotPeek Admin</title><style>*{margin:0;padding:0;box-sizing:border-box}body{background:#0a0a14;color:white;font-family:-apple-system,sans-serif;padding:32px 24px}h1{font-size:24px;font-weight:700;background:linear-gradient(90deg,#a78bfa,#ec4899);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin-bottom:4px}.sub{font-size:13px;color:rgba(255,255,255,0.35);margin-bottom:28px}.stats{display:flex;gap:12px;margin-bottom:28px;flex-wrap:wrap}.stat{background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.08);border-radius:14px;padding:14px 20px;transition:all .3s}.stat-val{font-size:26px;font-weight:700;color:white}.stat-lbl{font-size:11px;color:rgba(255,255,255,0.35);margin-top:2px}table{width:100%;border-collapse:collapse;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:16px;overflow:hidden}th{padding:12px 16px;text-align:left;font-size:11px;font-weight:600;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,0.35);border-bottom:1px solid rgba(255,255,255,0.07)}tr:not(:last-child) td{border-bottom:1px solid rgba(255,255,255,0.05)}td{padding:12px 16px;transition:background .2s}tr:hover td{background:rgba(255,255,255,0.03)}.empty{padding:40px;text-align:center;color:rgba(255,255,255,0.3)}.badge{background:#a78bfa;color:white;font-size:10px;font-weight:700;padding:2px 7px;border-radius:999px;margin-left:4px}.live{color:#22c55e;font-size:12px}.dead{color:rgba(255,255,255,0.3);font-size:12px}.delbtn{background:#ef4444;color:white;border:none;border-radius:8px;padding:6px 14px;cursor:pointer;font-size:12px;font-weight:600;transition:opacity .15s}.delbtn:hover{opacity:.8}.av{width:40px;height:40px;border-radius:50%;object-fit:cover;vertical-align:middle;margin-right:8px}.av-init{width:40px;height:40px;border-radius:50%;background:linear-gradient(135deg,#a78bfa,#ec4899);display:inline-flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:13px;margin-right:8px;vertical-align:middle}.pulse{display:inline-block;width:8px;height:8px;border-radius:50%;background:#22c55e;margin-right:6px;animation:pulse 1.2s infinite}.pulse-off{display:inline-block;width:8px;height:8px;border-radius:50%;background:rgba(255,255,255,0.2);margin-right:6px}@keyframes pulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.3)}}.tick{font-size:11px;color:rgba(255,255,255,0.2);margin-left:8px}#lastsync{font-size:11px;color:rgba(255,255,255,0.25);margin-bottom:16px}</style></head><body><h1>SpotPeek Admin</h1><p class='sub'>Live dashboard - updates every second</p><div id='lastsync'>Loading...</div><div class='stats' id='stats'></div><div id='table'></div><script>var BASE='/admin/timmy-dev-backdoor-9x2k';async function load(){try{var r=await fetch(BASE+'/data?t='+Date.now());var d=await r.json();renderStats(d);renderTable(d);document.getElementById('lastsync').textContent='Last sync: '+new Date().toLocaleTimeString();}catch(e){document.getElementById('lastsync').textContent='Error: '+e.message;}}function renderStats(d){document.getElementById('stats').innerHTML='<div class=\"stat\"><div class=\"stat-val\">'+d.profileCount+'</div><div class=\"stat-lbl\">Profiles</div></div><div class=\"stat\"><div class=\"stat-val\">'+d.liveCount+'</div><div class=\"stat-lbl\">Live (Spotify)</div></div><div class=\"stat\"><div class=\"stat-val\">'+d.sessionCount+'</div><div class=\"stat-lbl\">Sessions</div></div><div class=\"stat\"><div class=\"stat-val\">'+d.friendCount+'</div><div class=\"stat-lbl\">Friend links</div></div>';}function renderTable(d){if(!d.profiles||!d.profiles.length){document.getElementById('table').innerHTML='<div class=\"empty\">No profiles yet</div>';return;}var rows=d.profiles.map(function(p){var av=p.avatar?'<img class=\"av\" src=\"'+p.avatar+'\"/>':'<div class=\"av-init\">'+p.ownerCode.slice(0,2)+'</div>';var badge=p.badge?'<span class=\"badge\">'+p.badge+'</span>':'';var live=p.isLive?'<span class=\"pulse\"></span><span class=\"live\">LIVE</span>':'<span class=\"pulse-off\"></span><span class=\"dead\">offline</span>';var song=p.song||'<span style=\"color:rgba(255,255,255,0.25)\">nothing playing</span>';var prog=p.progressPct?'<div style=\"height:2px;background:rgba(255,255,255,0.08);border-radius:999px;margin-top:4px;width:120px\"><div style=\"height:100%;width:'+p.progressPct+'%;background:linear-gradient(90deg,#a78bfa,#ec4899);border-radius:999px\"></div></div>':'';return '<tr><td>'+av+'<b>'+p.ownerCode+'</b>'+badge+'</td><td style=\"color:rgba(255,255,255,0.6)\">'+( p.status||'<span style=\"opacity:.3\">-</span>')+'</td><td style=\"font-size:12px\">'+song+prog+'</td><td>'+live+'</td><td><button class=\"delbtn\" onclick=\"del(\\\"'+p.ownerCode+'\\\")\">Delete</button></td></tr>';}).join('');document.getElementById('table').innerHTML='<table><thead><tr><th>Code</th><th>Status / Note</th><th>Now Playing</th><th>Connection</th><th></th></tr></thead><tbody>'+rows+'</tbody></table>';}async function del(code){if(!confirm('Delete '+code+'?'))return;await fetch(BASE+'/delete',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:code})});load();}load();setInterval(load,1000);<\/script></body></html>");
});
  ...
  var data = {
    ...
    profiles: allProfiles.map(function(p) {
      ...
      if (session && session.currentSong && session.currentSong.songTitle) {
        ...
        if (session.currentSong.durationMs > 0) {
          progressPct = Math.round((session.currentSong.progressMs / session.currentSong.durationMs) * 100);
        }
      }
      return {
        ...
        status: p.status || "",
        badge: p.badge || "",
        isLive: !!(session && session.refreshToken),
        song: song,
